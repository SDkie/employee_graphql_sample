...
var _ = Describe("updateEmployee", func() {

	type updateEmployeeResponse struct {
		Data struct {
			Employee data.Employee `json:"updateEmployee"`
		} `json:"data"`
	...
	Context("Sending valid graphql query", func() {
		...
		It("should be updated successfully", func() {
			query := `
		mutation {
			updateEmployee(EMPNO:%d ENAME:"%s", JOB:"%s", MGR:%d, SALARY:%f, DEPTNO:%d){
				ENAME,
				JOB,
				MGR,
				SALARY,
			...
			// Change Name, Job and Dept
			emp.EName = "QWERTY_2"
			emp.Job = "NewJob"
			emp.DeptNo = dept.DeptNo
			query = fmt.Sprintf(query, emp.EmpNo, emp.EName, emp.Job, emp.Mgr, emp.Salary, dept.DeptNo)
	...
	Context("Not Sending EmpNo", func() {

		It("should fail", func() {
			query := `
		mutation {
			updateEmployee(ENAME:"%s", JOB:"%s", MGR:%d, SALARY:%f, DEPTNO:%d){
				ENAME,
				JOB,
				MGR,
				SALARY,
